...
export default function NewsletterForm() {
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="relative bg-white p-8 rounded-xl shadow-xl border border-gray-100">
          <div className="flex flex-col items-center justify-center">
            ...
            <p className="text-lg text-gray-800 font-medium">Thanks! We'll be in touch!</p>
          </div>
        </div>
  ...
  return (
    <div className="relative w-full max-w-md">
      <div className="absolute inset-0 bg-gradient-to-r from-teal-400 to-blue-500 rounded-2xl blur-lg opacity-20 animate-pulse"></div>
      <form className="relative bg-white p-2 rounded-xl shadow-xl border border-gray-100" onSubmit={handleSubmit}>
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <button
            type="submit"
            className="px-3 py-3 bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-white font-semibold rounded-lg transition-all transform hover:scale-105 shadow-lg hover:shadow-xl whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
            disabled={status === 'loading'}
          >
      ...
      {status === 'error' && (
        <div className="flex items-center justify-center py-4">
          <p className="text-sm text-red-600">{errorMessage || 'Oops! Something went wrong, please try again'}</p>
        </div>
      )}
      <p className="text-xs text-gray-500 mt-4 text-center">
        No spam, unsubscribe at any time. We respect your privacy.
