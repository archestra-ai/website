  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("###")) {
      ...
      if (categoryMatch) {
        ...
        rawCategory = rawCategory
          .replace(/^[\u{FE0F}\u{200D}️‍💻️️]+\s*/gu, "")
          .trim();
...
function convertToMCPServer(parsed: ParsedServer, index: number): MCPServer {
  ...
  if (
    parsed.description.toLowerCase().includes("stable") ||
    parsed.description.toLowerCase().includes("production")
  ) {
    status = "stable";
  ...
  if (descLower.includes("database") || descLower.includes("sql"))
    features.push("Database Operations");
  if (descLower.includes("file") || descLower.includes("filesystem"))
    features.push("File Operations");
  if (descLower.includes("web") || descLower.includes("http"))
    features.push("Web Requests");
  ...
  if (descLower.includes("realtime") || descLower.includes("real-time"))
